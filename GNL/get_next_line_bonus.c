/* ************************************************************************** */
/*                                                                            */
/*                                                        :::      ::::::::   */
/*   get_next_line_bonus.c                              :+:      :+:    :+:   */
/*                                                    +:+ +:+         +:+     */
/*   By: bokim <bokim@student.42.fr>                +#+  +:+       +#+        */
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2025/12/04 13:18:13 by bokim             #+#    #+#             */
/*   Updated: 2025/12/04 13:47:02 by bokim            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "get_next_line_bonus.h"

char	*get_next_line(int fd)
{
	static char	rest[MAX_FD][BUFFER_SIZE + 1];
	char		*line;
	int			nl;

	if (fd < 0 || fd >= 1024 || BUFFER_SIZE <= 0)
		return (0);
	nl = -1;
	line = ft_make_line(rest[fd], &nl);
	if (!line)
		return (0);
	ft_strlcpy(rest[fd], &rest[fd][nl + 1], BUFFER_SIZE + 1);
	line = ft_get_line(line, rest[fd], &nl, fd);
	if (!line || !line[0])
	{
		free(line);
		return (0);
	}
	return (line);
}

char	*ft_make_line(char *rest, int *nl)
{
	size_t	i;
	char	*line;

	i = 0;
	while (rest[i] && rest[i] != '\n')
		i++;
	i++;
	line = malloc(sizeof(char) * (i + 1));
	if (!line)
		return (0);
	ft_memcpy(line, rest, i);
	line[i] = '\0';
	if (i > 0 && line[i - 1] == '\n')
		*nl = i - 1;
	return (line);
}

char	*ft_get_line(char *line, char *rest, int *nl, int fd)
{
	char	buf[BUFFER_SIZE + 1];
	ssize_t	n;
	size_t	line_size;

	while (*nl == -1)
	{
		ft_bzero(buf, (BUFFER_SIZE + 1));
		n = read(fd, buf, BUFFER_SIZE);
		if (n == -1)
		{
			free(line);
			ft_bzero(rest, (BUFFER_SIZE + 1));
			return (0);
		}
		line_size = ft_find_nl(buf);
		ft_strlcpy(rest, &buf[line_size], (BUFFER_SIZE + 1));
		buf[line_size] = '\0';
		line = ft_strjoin(line, buf, nl);
		if (n == 0)
		{
			ft_bzero(rest, BUFFER_SIZE + 1);
			break ;
		}
	}
	return (line);
}

size_t	ft_find_nl(char *line)
{
	size_t	i;

	i = 0;
	while (i < BUFFER_SIZE)
	{
		if (line[i] == '\n' || !line[i])
			return (i + 1);
		i++;
	}
	return (i);
}
